...
function LoginScreen() {
  ...
  return (
    ...
    >
      {/* Outer card with gold glow border */}
      <div
      ...
      >
        {/* ── LEFT PANEL (55%) ── */}
        <div
          className="relative z-10 flex flex-col justify-between w-full md:w-[55%] px-8 py-10 flex-shrink-0"
        ...
        >
          ...
          <div>
            ...
            {forgotMode ? (
              ...
              <div>
                <button
                  onClick={() => { setForgotMode(false); setMsg({ text: '', ok: false }); }}
                  className="flex items-center gap-1.5 text-xs mb-6 transition-opacity hover:opacity-80"
                  style={{ color: 'rgba(234,179,8,0.6)' }}
                >
                  ← Back to login
                </button>
                <p style={{ fontFamily: "'Cinzel', serif", fontSize: '1.05rem', color: '#fff', fontWeight: 600, marginBottom: 6 }}>
                  Reset Password
                </p>
              ...
              <div>
                  ...
                  {['login', 'signup'].map(m => (
                    <button
                      ...
                      style={{
                        ...
                        color: mode === m ? '#EAB308' : 'rgba(255,255,255,0.3)',
                      }}
                    >
                ...
                <form onSubmit={handleAuth} className="space-y-5">
                  ...
                  <button
                    ...
                    className="w-full py-3.5 rounded-xl font-bold text-sm text-black active:scale-[0.98] transition-all disabled:opacity-50"
                    style={{
                      background: 'linear-gradient(135deg, #EAB308 0%, #ca9a07 100%)',
                  ...
                  {mode === 'login' && (
                    <button
                      type="button"
                      onClick={() => { setForgotMode(true); setMsg({ text: '', ok: false }); }}
        ...
        {/* ── RIGHT PANEL (45%) — hidden on mobile ── */}
        <div
          className="hidden md:flex relative flex-col items-center justify-center flex-1 overflow-hidden"
          style={{
            background: 'linear-gradient(160deg, #1a1200 0%, #2d1f00 60%, #1a1200 100%)',
            clipPath: 'polygon(8% 0%, 100% 0%, 100% 100%, 0% 100%)',
          }}
        >
          {/* Thin glowing gold line at the diagonal edge */}
          <div
