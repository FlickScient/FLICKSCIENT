...
function LoginScreen() {
  ...
  const inputCls = "w-full px-4 py-3.5 rounded-2xl border outline-none transition-all text-sm text-white placeholder-gray-500 bg-white/5 border-white/10 focus:border-[#E8600A]/60 focus:ring-2 focus:ring-[#E8600A]/20";
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden p-5"
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden>
        <div className="mesh-blob-1 absolute w-[500px] h-[500px] rounded-full opacity-30"
          style={{ background: 'radial-gradient(circle, #E8600A 0%, transparent 70%)', top: '-10%', left: '-10%', filter: 'blur(80px)' }} />
        <div className="mesh-blob-2 absolute w-[600px] h-[600px] rounded-full opacity-20"
          style={{ background: 'radial-gradient(circle, #7c3aed 0%, transparent 70%)', bottom: '-15%', right: '-15%', filter: 'blur(100px)' }} />
        <div className="mesh-blob-3 absolute w-[400px] h-[400px] rounded-full opacity-20"
          style={{ background: 'radial-gradient(circle, #0ea5e9 0%, transparent 70%)', top: '40%', left: '50%', filter: 'blur(80px)' }} />
        <div className="mesh-blob-1 absolute w-[300px] h-[300px] rounded-full opacity-15"
          style={{ background: 'radial-gradient(circle, #E8600A 0%, transparent 70%)', bottom: '10%', left: '15%', filter: 'blur(60px)', animationDelay: '-5s' }} />
      </div>
      ...
      {/* Main content */}
      <div className="relative z-10 w-full max-w-sm login-card-enter">

        {/* Branding */}
        <div className="text-center mb-8">
          <div className="relative inline-flex mb-4">
            <div className="w-16 h-16 rounded-2xl flex items-center justify-center"
              style={{ background: 'rgba(232,96,10,0.15)', border: '1px solid rgba(232,96,10,0.3)', boxShadow: '0 0 50px rgba(232,96,10,0.25), inset 0 1px 0 rgba(255,255,255,0.1)' }}>
              <MovieSyncLogo size={34} className="text-[#E8600A]" />
            </div>
            <div className="absolute -inset-2 rounded-3xl opacity-40 blur-xl"
              style={{ background: 'radial-gradient(circle, rgba(232,96,10,0.6) 0%, transparent 70%)' }} />
          </div>
          <h1 className="text-2xl font-black text-white tracking-tight">
            Movie<span style={{ color: '#E8600A' }}>Sync</span>
          </h1>
          <p className="text-[10px] mt-1 uppercase tracking-[0.35em] font-bold" style={{ color: 'rgba(255,255,255,0.3)' }}>
            The Ultimate Canon
          </p>
        </div>
          ...
          {forgotMode ? (
            <>
              <button onClick={() => { setForgotMode(false); setMsg({ text:'', ok:false }); }}
                className="flex items-center gap-1.5 text-xs hover:opacity-80 mb-5 transition-opacity"
              ...
              <form onSubmit={handleForgot} className="space-y-3">
                ...
                <button type="submit" disabled={loading}
                  className="w-full font-black py-4 rounded-2xl active:scale-[0.98] transition-all disabled:opacity-50 text-sm text-white"
                  style={{ background: 'linear-gradient(135deg, #E8600A 0%, #f07020 100%)', boxShadow: '0 4px 24px rgba(232,96,10,0.35)' }}>
            ...
            <>
              {/* Pill tab switcher with glow */}
              <div className="flex p-1 mb-6 rounded-2xl" style={{ background: 'rgba(0,0,0,0.35)', border: '1px solid rgba(255,255,255,0.06)' }}>
                {['login','signup'].map(m => (
                  <button key={m} onClick={() => { setMode(m); setMsg({ text:'', ok:false }); }}
                    className="flex-1 py-2.5 rounded-xl text-sm font-black transition-all"
                    style={mode === m ? {
                      background: 'linear-gradient(135deg, #E8600A 0%, #f07020 100%)',
                      color: '#ffffff',
                      boxShadow: '0 2px 16px rgba(232,96,10,0.40)',
              ...
              <button onClick={handleGoogle} disabled={loading}
                className="w-full flex items-center justify-center gap-3 bg-white text-gray-900 font-bold py-3.5 rounded-2xl mb-5 hover:bg-gray-50 active:scale-[0.98] transition-all disabled:opacity-50 text-sm"
              ...
              <form onSubmit={handleAuth} className="space-y-3">
                ...
                <button type="submit" disabled={loading}
                  className="w-full font-black py-4 rounded-2xl active:scale-[0.98] transition-all disabled:opacity-50 text-sm text-white"
                  style={{ background: 'linear-gradient(135deg, #E8600A 0%, #f07020 100%)', boxShadow: '0 4px 28px rgba(232,96,10,0.40)' }}>
                  {loading ? '···' : mode === 'login' ? 'Log In' : 'Create Account'}
                </button>

                {mode === 'login' && (
                  <button type="button" onClick={() => { setForgotMode(true); setMsg({ text:'', ok:false }); }}
                    className="w-full text-center text-[11px] transition-opacity hover:opacity-80 pt-1"
        ...
        <p className="text-center text-[10px] mt-6" style={{ color: 'rgba(255,255,255,0.2)' }}>
          Made by <span style={{ color: 'rgba(232,96,10,0.6)', fontWeight: 700 }}>Mahmudul Hasan Mahid</span>
        </p>
      </div>
    </div>
  );
...
function DrawerMenu({ open, onClose, user, onLogout, onOpenSeed, theme, onToggleTheme }) {
  ...
  return (
    <>
      <DrawerMenu open={drawerOpen} onClose={() => setDrawerOpen(false)}
        ...
        theme={theme} onToggleTheme={() => setTheme(v => v === 'dark' ? 'light' : v === 'light' ? 'maestro' : 'dark')} />
